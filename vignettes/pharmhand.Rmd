...
That's it! The functions automatically:
- Accept data.frames directly (no S7 wrapping needed)
- Use sensible defaults (configurable via `options()`)
- Create formatted Word documents with multiple tables

## Core Workflow (Detailed Control)

For more control, build tables and reports step-by-step:

> **Note:** Scalar inputs must be non-missing. NA values should be removed or checked using `assert_no_na()` before passing to validation functions.
...
Available options:
- `pharmhand.trt_var` - Treatment variable (default: "TRT01P")
- `pharmhand.subject_var` - Subject ID variable (default: "USUBJID")
