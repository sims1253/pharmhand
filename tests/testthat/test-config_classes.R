...
test_that("Configuration class is properly defined", {
...
test_that("Configuration properties have correct defaults", {
...
test_that("Configuration source can be overridden", {
	subgroup <- SubgroupConfig(
		variable = "AGEGR1",
...
test_that("SubgroupConfig can be created with required variable", {
...
test_that("SubgroupConfig with all parameters", {
...
test_that("SubgroupConfig has correct defaults", {
...
test_that("SubgroupConfig inherits from Configuration", {
...
test_that("SubgroupConfig labels can be empty list", {
	config <- SubgroupConfig(
		variable = "CUSTOM_VAR",
		labels = list()
	)
...
test_that("SubgroupConfig order is optional", {
	config <- SubgroupConfig(
		variable = "AGEGR1",
		order = NULL
	)
...
test_that("PopulationConfig can be created with required variable", {
...
test_that("PopulationConfig with all parameters", {
	config <- PopulationConfig(
		variable = "SAFFL",
		label = "Safety Analysis Set",
...
test_that("PopulationConfig has correct defaults", {
	config <- PopulationConfig(
		variable = "FASFL",
...
test_that("PopulationConfig inherits from Configuration", {
	config <- PopulationConfig(
		variable = "PPSFL",
...
test_that("PopulationConfig flag_value can be customized", {
	config <- PopulationConfig(
		variable = "CUSTOM_FL",
...
test_that("PopulationConfig description can be NULL", {
	config <- PopulationConfig(
		variable = "TESTFL",
		label = "Test",
		description = NULL
...
test_that("SOCConfig can be created with required variable", {
...
test_that("SOCConfig with all parameters", {
	config <- SOCConfig(
		variable = "AEBODSYS",
		include_all = TRUE,
...
test_that("SOCConfig has correct defaults", {
	config <- SOCConfig(variable = "AEBODSYS")
...
test_that("SOCConfig inherits from Configuration", {
	config <- SOCConfig(variable = "AEBODSYS")
...
test_that("SOCConfig custom_order can be NULL", {
	config <- SOCConfig(
		variable = "AEBODSYS",
		custom_order = NULL
	)
...
test_that("SOCConfig sort_by accepts 'frequency' and 'alphabetical'", {
...
test_that("SOCConfig top_n can be set to limit SOCs", {
...
test_that("PTConfig can be created with required variable", {
...
test_that("PTConfig with all parameters", {
	config <- PTConfig(
		variable = "AEDECOD",
		include_all = FALSE,
		sort_by = "alphabetical",
...
test_that("PTConfig has correct defaults", {
	config <- PTConfig(variable = "AEDECOD")
...
test_that("PTConfig inherits from Configuration", {
	config <- PTConfig(variable = "AEDECOD")
...
test_that("PTConfig show_pt_codes defaults to FALSE", {
	config <- PTConfig(variable = "AEDECOD")

	expect_equal(config@show_pt_codes, FALSE)
})

test_that("PTConfig top_n_per_soc can limit PTs per SOC", {
	config <- PTConfig(
		variable = "AEDECOD",
		top_n_per_soc = 10
	)
...
test_that("ConfigurationRegistry can be created empty", {
...
test_that("ConfigurationRegistry with subgroups and populations", {
	subgroup <- SubgroupConfig(
		variable = "AGEGR1",
		labels = list(Y_LT65 = "< 65")
	)
...
test_that("ConfigurationRegistry with SOC and PT configs", {
...
test_that("ConfigurationRegistry has correct defaults for all properties", {
	registry <- ConfigurationRegistry()
...
test_that("ConfigurationRegistry can hold complex nested settings", {
	registry <- ConfigurationRegistry(
		performance = list(
...
test_that("ConfigurationRegistry can have multiple subgroups", {
...
test_that("ConfigurationRegistry can have multiple populations", {
...
test_that("ConfigurationRegistry inherits from Configuration", {
	registry <- ConfigurationRegistry()
...
test_that("All config classes have proper class hierarchy", {
	# Create instances
...
test_that("Properties are accessible via @ operator", {
...
test_that("Configuration properties can be modified", {
	config <- PopulationConfig(
		variable = "SAFFL",
		label = "Safety",
		priority = 0L
...
test_that("SubgroupConfig handles complex label structures", {
	complex_labels <- list(
		AGEGRP1 = "< 65 years",
		AGEGRP2 = "65-74 years",
		AGEGRP3 = "75-84 years",
...
test_that("PopulationConfig handles character flag_value", {
	config <- PopulationConfig(
		variable = "CUSTOMFL",
		label = "Custom",
		flag_value = "1"
...
test_that("SOCConfig handles numeric min_subjects", {
	config <- SOCConfig(
		variable = "AEBODSYS",
		min_subjects = 5
	)
...
test_that("ConfigurationRegistry validation property structure", {
	registry <- ConfigurationRegistry(
		validation = list(
			check_data = TRUE,
