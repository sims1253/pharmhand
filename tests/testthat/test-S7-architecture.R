...
describe("S7-architecture", {
	it("ADaMData and Analysis Core works", {
	...
	it("Baseline Characteristics calculation works", {
		adsl <- get_shared_adsl(n = 20)
	...
	it("Reporting Engine (flextable/gt) works", {
	...
	it("AnalysisResults computed properties work with empty stats", {
	...
	it("ADaMData filtered_data computed property works", {
		adsl <- get_shared_adsl(n = 20)
	...
	it("ADaMData filtered_data returns all for ALL population", {
		adsl <- get_shared_adsl(n = 20)
	...
	it("ADaMData trt_n computed property works", {
		adsl <- get_shared_adsl(n = 20)
		adam <- ADaMData(data = adsl, population = "ITT")
	...
	it("ADaMData trt_n respects population filter", {
		adsl <- get_shared_adsl(n = 20)
	...
	it("ADaMData subject_n computed property works", {
		adsl <- get_shared_adsl(n = 20)
		adam <- ADaMData(data = adsl, population = "ITT")
	...
	it("ADaMData subject_n handles empty filtered_data", {
		empty_df <- data.frame(
			USUBJID = character(),
			TRT01P = character(),
			ITTFL = character()
	...
	it("ADaMData trt_levels computed property works", {
		adsl <- get_shared_adsl(n = 20)
		adam <- ADaMData(data = adsl, population = "ITT")
	...
	it("ADaMData trt_levels handles empty filtered_data", {
		empty_df <- data.frame(
			USUBJID = character(),
			TRT01P = character(),
			ITTFL = character()
	...
	it("ADaMData is_empty computed property works", {
		adsl <- get_shared_adsl(n = 20)
		adam <- ADaMData(data = adsl, population = "ITT")
	...
	it("ADaMData is_empty returns TRUE for empty filtered_data", {
		empty_df <- data.frame(
			USUBJID = character(),
			TRT01P = character(),
			ITTFL = character()
	...
	it("ADaMData summary_label computed property works", {
		adsl <- get_shared_adsl(n = 20)
	...
	it("ADaMData summary_label handles empty data", {
		empty_df <- data.frame(
			USUBJID = character(),
			TRT01P = character(),
			FASFL = character()
	...
	it("get_trt_n works with ADaMData", {
		adsl <- get_shared_adsl(n = 20)
		adam <- ADaMData(data = adsl, population = "ITT")
	...
	it("get_trt_n works with data frame", {
		adsl <- get_shared_adsl(n = 20)
	...
	it("get_trt_n applies population filter for data frames", {
		adsl <- get_shared_adsl(n = 20)
		adsl$PPSFL <- "Y"
		adsl$PPSFL[1:4] <- "N"
	...
	it("get_filtered_data works with ADaMData", {
		adsl <- get_shared_adsl(n = 20)
		adam <- ADaMData(data = adsl, population = "ITT")
	...
	it("get_filtered_data works with data frame", {
		adsl <- get_shared_adsl(n = 20)
		adsl$PPSFL <- "Y"
		adsl$PPSFL[1:4] <- "N"
	...
	it("get_trt_var returns correct variable", {
		adsl <- get_shared_adsl(n = 20)
	...
	it("get_subject_var returns correct variable", {
		adsl <- get_shared_adsl(n = 20)
		adam <- ADaMData(data = adsl)
	...
	it("get_subject_n works with ADaMData", {
		adsl <- get_shared_adsl(n = 20)
		adam <- ADaMData(data = adsl, population = "ITT")
	...
	it("get_subject_n works with data frame", {
		adsl <- get_shared_adsl(n = 20)

		expect_equal(get_subject_n(adsl), 20L)
	})

	it("get_subject_n applies population filter for data frames", {
		adsl <- get_shared_adsl(n = 20)
		adsl$PPSFL <- "Y"
		adsl$PPSFL[1:4] <- "N"
	...
	it("get_summary_label works with ADaMData", {
		adsl <- get_shared_adsl(n = 20)
		adam <- ADaMData(data = adsl, population = "FAS")
	...
	it("get_summary_label works with AnalysisResults", {
		adsl <- get_shared_adsl(n = 20)
		adam <- ADaMData(data = adsl)
		results <- calculate_baseline(adam, vars = c("AGE"))
	...
	it("get_summary_label works with empty AnalysisResults", {
		empty_results <- AnalysisResults(
			stats = data.frame(),
			type = "test"
		)
