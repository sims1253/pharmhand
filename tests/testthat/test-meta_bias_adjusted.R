...
describe("meta bias adjusted", {
	it("calculate_rob_weights returns list with required components", {
	...
	it("calculate_rob_weights applies correct multipliers for RoB 2 Low", {
		meta_res <- meta_analysis(
			yi = .meta_yi_hr,
			sei = .meta_sei_hr,
			study_labels = paste("Study", 1:5),
	...
	it("calculate_rob_weights applies multipliers for RoB 2 concerns", {
		meta_res <- meta_analysis(
			yi = .meta_yi_hr,
			sei = .meta_sei_hr,
			study_labels = paste("Study", 1:5),
	...
	it("calculate_rob_weights applies correct multipliers for RoB 2 High", {
		meta_res <- meta_analysis(
			yi = .meta_yi_hr,
			sei = .meta_sei_hr,
			study_labels = paste("Study", 1:5),
	...
	it("calculate_rob_weights applies correct multipliers for ROBINS-I", {
		meta_res <- meta_analysis(
			yi = .meta_yi_hr,
			sei = .meta_sei_hr,
			study_labels = paste("Study", 1:5),
	...
	it("calculate_rob_weights normalizes ROB2 weights to sum to 1", {
		meta_res <- meta_analysis(
			yi = .meta_yi_hr,
			sei = .meta_sei_hr,
			study_labels = paste("Study", 1:5),
	...
	it("calculate_rob_weights handles unassessed studies", {
		meta_res <- meta_analysis(
			yi = .meta_yi_hr,
			sei = .meta_sei_hr,
			study_labels = paste("Study", 1:5),
	...
	it("calculate_rob_weights warns when all weights are zero", {
		meta_res <- meta_analysis(
			yi = .meta_yi_hr,
			sei = .meta_sei_hr,
			study_labels = paste("Study", 1:5),
	...
	it("calculate_rob_weights errors with invalid meta_result", {
		rob_results <- list(
			assess_rob2("Study 1", "Low", "Low", "Low", "Low", "Low")
		)
	...
	it("calculate_rob_weights errors with empty rob_results", {
		meta_res <- meta_analysis(
			yi = .meta_yi_hr,
			sei = .meta_sei_hr,
			effect_measure = "hr"
	...
	it("calculate_rob_weights errors with invalid RoB result", {
		meta_res <- meta_analysis(
			yi = .meta_yi_hr,
			sei = .meta_sei_hr,
			study_labels = paste("Study", 1:5),
	...
	it("rob_sensitivity_analysis returns list with required components", {
		meta_res <- meta_analysis(
			yi = .meta_yi_hr,
			sei = .meta_sei_hr,
			study_labels = paste("Study", 1:5),
	...
	it("rob_sensitivity_analysis runs Low risk only scenario for RoB 2", {
		meta_res <- meta_analysis(
			yi = .meta_yi_hr,
			sei = .meta_sei_hr,
			study_labels = paste("Study", 1:5),
	...
	it("rob_sensitivity_analysis runs Low + Some concerns scenario", {
		meta_res <- meta_analysis(
			yi = .meta_yi_hr,
			sei = .meta_sei_hr,
			study_labels = paste("Study", 1:5),
	...
	it("rob_sensitivity_analysis runs Excluding High risk scenario", {
		meta_res <- meta_analysis(
			yi = .meta_yi_hr,
			sei = .meta_sei_hr,
			study_labels = paste("Study", 1:5),
	...
	it("rob_sensitivity_analysis works with ROBINS-I assessments", {
		meta_res <- meta_analysis(
			yi = .meta_yi_hr,
			sei = .meta_sei_hr,
			study_labels = paste("Study", 1:5),
	...
	it("rob_sensitivity_analysis calculates percent change from original", {
		meta_res <- meta_analysis(
			yi = .meta_yi_hr,
			sei = .meta_sei_hr,
			study_labels = paste("Study", 1:5),
	...
	it("rob_sensitivity_analysis handles insufficient studies", {
		meta_res <- meta_analysis(
			yi = c(0.5, 0.8),
			sei = c(0.1, 0.2),
			effect_measure = "md"
	...
	it("rob_sensitivity_analysis errors with invalid meta_result", {
		rob_results <- list(
			assess_rob2("Study 1", "Low", "Low", "Low", "Low", "Low")
		)
	...
	it("rob_sensitivity_analysis errors with empty rob_results", {
		meta_res <- meta_analysis(
			yi = .meta_yi_hr,
			sei = .meta_sei_hr,
			effect_measure = "hr"
	...
	it("bias_adjusted_meta returns BiasAdjustedMetaResult", {
		meta_res <- meta_analysis(
			yi = .meta_yi_hr,
			sei = .meta_sei_hr,
			study_labels = paste("Study", 1:5),
	...
	it("bias_adjusted_meta with weight_downgrade adjusts weights", {
		meta_res <- meta_analysis(
			yi = .meta_yi_hr,
			sei = .meta_sei_hr,
			study_labels = paste("Study", 1:5),
	...
	it("bias_adjusted_meta with exclude_high removes high risk studies", {
		meta_res <- meta_analysis(
			yi = .meta_yi_hr,
			sei = .meta_sei_hr,
			study_labels = paste("Study", 1:5),
	...
	it("bias_adjusted_meta with selection_model adjusts weights", {
		meta_res <- meta_analysis(
			yi = .meta_yi_hr,
			sei = .meta_sei_hr,
			study_labels = paste("Study", 1:5),
	...
	it("bias_adjusted_meta works with ROBINS-I assessments", {
		meta_res <- meta_analysis(
			yi = .meta_yi_hr,
			sei = .meta_sei_hr,
			study_labels = paste("Study", 1:5),
	...
	it("bias_adjusted_meta stores adjustment metadata", {
		meta_res <- meta_analysis(
			yi = .meta_yi_hr,
			sei = .meta_sei_hr,
			study_labels = paste("Study", 1:5),
	...
	it("bias_adjusted_meta errors with insufficient studies", {
		meta_res <- meta_analysis(
			yi = .meta_yi_hr,
			sei = .meta_sei_hr,
			study_labels = paste("Study", 1:5),
	...
	it("bias_adjusted_meta errors with invalid meta_result", {
		rob_results <- list(
			assess_rob2("Study 1", "Low", "Low", "Low", "Low", "Low")
		)
	...
	it("bias_adjusted_meta errors with empty rob_results", {
		meta_res <- meta_analysis(
			yi = .meta_yi_hr,
			sei = .meta_sei_hr,
			effect_measure = "hr"
	...
	it("bias_adjusted_meta errors with mismatched method", {
		meta_res <- meta_analysis(
			yi = .meta_yi_hr,
			sei = .meta_sei_hr,
			study_labels = paste("Study", 1:5),
	...
	it("summarize_bias_adjusted returns data frame with comparison", {
		meta_res <- meta_analysis(
			yi = .meta_yi_hr,
			sei = .meta_sei_hr,
			study_labels = paste("Study", 1:5),
	...
	it("summarize_bias_adjusted calculates percent change", {
		meta_res <- meta_analysis(
			yi = .meta_yi_hr,
			sei = .meta_sei_hr,
			study_labels = paste("Study", 1:5),
	...
	it("summarize_bias_adjusted provides interpretation", {
		meta_res <- meta_analysis(
			yi = .meta_yi_hr,
			sei = .meta_sei_hr,
			study_labels = paste("Study", 1:5),
	...
	it("summarize_bias_adjusted errors with invalid adjusted_result", {
		meta_res <- meta_analysis(
			yi = .meta_yi_hr,
			sei = .meta_sei_hr,
			effect_measure = "hr"
	...
	it("summarize_bias_adjusted errors with invalid original_result", {
		meta_res <- meta_analysis(
			yi = .meta_yi_hr,
			sei = .meta_sei_hr,
			study_labels = paste("Study", 1:5),
	...
	it("calculate_rob_weights integrates with RoB2Result", {
		meta_res <- meta_analysis(
			yi = .meta_yi_hr,
			sei = .meta_sei_hr,
			study_labels = paste("Study", 1:5),
	...
	it("calculate_rob_weights integrates with ROBINSIResult", {
		meta_res <- meta_analysis(
			yi = .meta_yi_hr,
			sei = .meta_sei_hr,
			study_labels = paste("Study", 1:5),
	...
	it("bias_adjusted_meta works with mixed RoB results", {
		meta_res <- meta_analysis(
			yi = .meta_yi_hr,
			sei = .meta_sei_hr,
			study_labels = paste("Study", 1:5),
	...
	it("rejects invalid weight parameters", {
		# Suppress REML convergence warning - not the focus of this test
		suppressWarnings({
			meta_res <- meta_analysis(
				yi = c(0.5, 0.6, 0.7),
