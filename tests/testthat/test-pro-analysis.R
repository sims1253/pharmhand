...
describe("pro-analysis", {
	it("calculate_mcid_anchor calculates anchor-based MCID", {
		data <- data.frame(
	...
	it("calculate_mcid_distribution calculates distribution-based MCID", {
		set.seed(123)
		data <- data.frame(
	...
	it("calculate_mcid_distribution calculates SEM with reliability", {
		set.seed(123)
		data <- data.frame(
			BASE = rnorm(100, mean = 50, sd = 10)
		)
	...
	it("calculate_mcid wrapper works with both methods", {
		data <- data.frame(
			CHG = c(-5, -3, -4, -6, -2, 0, 2, 5, 3, 4),
			PGIC = c(rep("Minimally Improved", 5), rep("No Change", 5)),
	...
	it("create_ttd_analysis creates time-to-deterioration analysis", {
	...
	it("create_mean_plot creates ClinicalPlot", {
		set.seed(123)
		data <- data.frame(
	...
	it("create_mean_plot calculates correct CIs", {
	...
	it("create_spider_plot creates ClinicalPlot", {
		set.seed(123)
		data <- data.frame(
	...
	it("create_spider_plot handles threshold lines", {
		set.seed(123)
		data <- data.frame(
	...
	it("calculate_mcid_anchor handles empty minimal group", {
		data <- data.frame(
			CHG = c(-5, -3, -4, -6, -2, 0, 2, 5, 3, 4),
	...
	it("calculate_mcid_anchor handles single observation", {
		data <- data.frame(
	...
	it("calculate_mcid_anchor handles NA scores", {
		data <- data.frame(
	...
	it("calculate_mcid_anchor with different anchor levels", {
		data <- data.frame(
	...
	it("calculate_mcid_anchor handles variable not found", {
		data <- data.frame(
			CHG = c(-5, -3, -4),
	...
	it("calculate_mcid_anchor with custom confidence level", {
		data <- data.frame(
			CHG = c(-5, -3, -4, -6, -2, 0, 2, 5, 3, 4),
			PGIC = c(rep("Minimally Improved", 5), rep("No Change", 5))
		)
	...
	it("calculate_mcid_distribution handles insufficient data", {
		data <- data.frame(
	...
	it("calculate_mcid_distribution handles single observation", {
		data <- data.frame(
			BASE = c(50)
		)
	...
	it("calculate_mcid_distribution handles all NA values", {
		data <- data.frame(
			BASE = c(NA, NA, NA)
		)
	...
	it("calculate_mcid_distribution handles mixed NA values", {
		data <- data.frame(
	...
	it("calculate_mcid_distribution handles zero variance", {
		data <- data.frame(
	...
	it("calculate_mcid_distribution with reliability (SEM)", {
		set.seed(123)
		data <- data.frame(
			BASE = rnorm(100, mean = 50, sd = 10)
		)
	...
	it("calculate_mcid_distribution without reliability (no SEM)", {
		set.seed(123)
		data <- data.frame(
			BASE = rnorm(100, mean = 50, sd = 10)
		)
	...
	it("calculate_mcid_distribution handles reliability edge cases", {
		set.seed(123)
		data <- data.frame(
			BASE = rnorm(100, mean = 50, sd = 10)
		)
	...
	it("calculate_mcid_distribution with selective methods", {
		set.seed(123)
		data <- data.frame(
			BASE = rnorm(100, mean = 50, sd = 10)
		)
	...
	it("calculate_mcid_distribution handles variable not found", {
		set.seed(123)
		data <- data.frame(
			OTHER = rnorm(100)
		)
	...
	it("calculate_mcid with anchor only", {
		data <- data.frame(
			CHG = c(-5, -3, -4, -6, -2, 0, 2, 5, 3, 4),
			PGIC = c(rep("Minimally Improved", 5), rep("No Change", 5))
		)
	...
	it("calculate_mcid with distribution only", {
		data <- data.frame(
			CHG = c(-5, -3, -4, -6, -2, 0, 2, 5, 3, 4)
		)
	...
	it("calculate_mcid requires anchor_var for anchor method", {
		data <- data.frame(
			CHG = c(-5, -3, -4)
		)
	...
	it("calculate_mcid uses score_var if baseline_var missing", {
		data <- data.frame(
			CHG = c(-5, -3, -4, -6, -2, 0, 2, 5, 3, 4)
		)
	...
	it("create_ttd_analysis handles no events", {
		set.seed(123)
		data <- expand.grid(
	...
	it("create_ttd_analysis handles all events", {
		set.seed(123)
		data <- expand.grid(
			USUBJID = paste0("SUBJ", sprintf("%03d", 1:20)),
			AVISITN = 0:3
	...
	it("create_ttd_analysis with increase direction", {
		set.seed(123)
		data <- expand.grid(
			USUBJID = paste0("SUBJ", sprintf("%03d", 1:20)),
			AVISITN = 0:3
	...
	it("create_ttd_analysis confirmed definition detects consecutive", {
		set.seed(123)
	...
	it("create_ttd_analysis confirmed definition uses individual CHG", {
		set.seed(123)
		data <- expand.grid(
	...
	it("create_ttd_analysis with custom censor_at", {
		set.seed(123)
		data <- expand.grid(
			USUBJID = paste0("SUBJ", sprintf("%03d", 1:20)),
			AVISITN = 0:3
