...
describe("module4-table", {
	it("create_hta_module4_table creates empty template", {
	...
	it("create_hta_module4_table adds missing columns", {
	...
	it("create_hta_module4_table drops extra columns by default", {
		df <- data.frame(
			Endpoint = "OS",
			Effect = 0.8,
			Extra = "drop"
	...
	it("create_hta_module4_table allows extra columns when requested", {
		df <- data.frame(
			Endpoint = "OS",
			Effect = 0.8,
			Extra = "keep"
	...
	it("create_hta_module4_table applies G-BA theme marker", {
		result <- create_hta_module4_table(
	...
	it("create_hta_module4_table validates empty columns vector", {
		expect_error(
