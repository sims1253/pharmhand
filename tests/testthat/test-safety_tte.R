...
describe("safety tte", {
	it("calculate_ae_tte_data returns valid TTE data with valid inputs", {
	...
	it("calculate_ae_tte_data derives TRTDURD from TRTEDT/TRTSDT", {
		adsl <- data.frame(
	...
	it("calculate_ae_tte_data errors with missing ADAE columns", {
	...
	it("calculate_ae_tte_data errors with missing ADSL columns", {
		adsl <- get_shared_adsl()
		adae <- data.frame(
			USUBJID = c("SUBJ001"),
			AEBODSYS = c("Nervous system"),
	...
	it("calculate_ae_tte_data errors when neither TRTDURD nor
	...
	it("calculate_ae_tte_data handles empty data", {
		adsl <- data.frame(
			USUBJID = character(0),
			SAFFL = character(0),
			TRT01P = character(0),
	...
	it("calculate_ae_tte_data handles subjects without events (censored)", {
		adsl <- data.frame(
	...
	it("calculate_ae_tte_data filters to SAFFL == 'Y'", {
		adsl <- data.frame(
			USUBJID = c("SUBJ001", "SUBJ002", "SUBJ003"),
			SAFFL = c("Y", "N", "Y"),
	...
	it("calculate_ae_tte_data takes first of multiple events/subject", {
		adsl <- data.frame(
			USUBJID = c("SUBJ001"),
			SAFFL = c("Y"),
			TRT01P = c("Placebo"),
	...
	it("calculate_ae_tte_data handles NA treatment values", {
		adsl <- data.frame(
			USUBJID = c("SUBJ001", "SUBJ002", "SUBJ003"),
			SAFFL = c("Y", "Y", "Y"),
	...
	it("calculate_ae_tte_data uses custom trt_var", {
		adsl <- data.frame(
			USUBJID = c("SUBJ001", "SUBJ002"),
			SAFFL = c("Y", "Y"),
	...
	it("create_ae_km_plot_for_soc returns ClinicalPlot with valid data", {
		adsl <- data.frame(
			USUBJID = c("SUBJ001", "SUBJ002", "SUBJ003", "SUBJ004"),
			SAFFL = c("Y", "Y", "Y", "Y"),
			TRT01P = c("Placebo", "Active", "Placebo", "Active"),
	...
	it("create_ae_km_plot_for_soc returns NULL with empty data", {
		adsl <- data.frame(
			USUBJID = character(0),
			SAFFL = character(0),
			TRT01P = character(0),
	...
	it("create_time_to_first_ae returns valid result with valid inputs", {
		adsl <- data.frame(
			USUBJID = c("SUBJ001", "SUBJ002", "SUBJ003", "SUBJ004"),
			SAFFL = c("Y", "Y", "Y", "Y"),
			TRT01P = c("Placebo", "Active", "Placebo", "Active"),
	...
	it("create_time_to_first_ae works without ae_filter", {
		set.seed(123)
	...
	it("create_time_to_first_ae errors with missing required ADAE columns", {
		adsl <- get_shared_adsl()
	...
	it("create_time_to_first_ae errors with missing required ADSL columns", {
		adsl <- data.frame(
			USUBJID = c("SUBJ001"),
			# Missing SAFFL
			TRT01P = c("Placebo"),
	...
	it("create_time_to_first_ae derives censor_var from TRTEDT/TRTSDT", {
		set.seed(123)
		# Use larger dataset to avoid coxph convergence warnings
		n_per_arm <- 20
		adsl <- data.frame(
	...
	it("create_time_to_first_ae errors when censor_var not derivable", {
		adsl <- data.frame(
			USUBJID = c("SUBJ001"),
			SAFFL = c("Y"),
			TRT01P = c("Placebo"),
	...
	it("create_time_to_first_ae filters to TRTEMFL == 'Y'", {
		set.seed(123)
	...
	it("create_time_to_first_ae errors with no subjects available", {
		set.seed(123)
	...
	it("create_time_to_first_ae handles custom time_var", {
		adsl <- data.frame(
			USUBJID = c("SUBJ001", "SUBJ002"),
			SAFFL = c("Y", "Y"),
	...
	it("create_time_to_first_ae handles custom censor_var", {
		adsl <- data.frame(
			USUBJID = c("SUBJ001", "SUBJ002"),
			SAFFL = c("Y", "Y"),
			TRT01P = c("Placebo", "Placebo"),
	...
	it("create_time_to_first_ae validates conf_level parameter", {
		adsl <- data.frame(
			USUBJID = c("SUBJ001"),
			SAFFL = c("Y"),
			TRT01P = c("Placebo"),
	...
	it("create_time_to_first_ae handles multiple events per subject", {
		set.seed(123)
		# Use larger dataset to avoid coxph convergence warnings
		n_per_arm <- 20
		adsl <- data.frame(
	...
	it("create_time_to_first_ae filters SAFFL == 'Y'", {
		set.seed(123)
	...
	it("create_time_to_first_ae uses custom title", {
		set.seed(123)
