...
test_that("create_analysis_meta populates timestamp with current time", {
	before <- Sys.time()
	meta <- create_analysis_meta()
	after <- Sys.time()
...
test_that("create_analysis_meta populates package_version", {
	meta <- create_analysis_meta()

	expect_true(nchar(meta@package_version) > 0)
})

test_that("create_analysis_meta populates r_version", {
	meta <- create_analysis_meta()
...
test_that("create_analysis_meta sets source_vars correctly", {
	meta <- create_analysis_meta(source_vars = c("AGE", "SEX", "TRT"))

	expect_equal(meta@source_vars, c("AGE", "SEX", "TRT"))
})

test_that("create_analysis_meta defaults source_vars to empty", {
	meta <- create_analysis_meta()

	expect_equal(meta@source_vars, character())
})

test_that("create_analysis_meta sets filters correctly", {
	filters <- list(
		population = "ITT",
		safl = "Y"
	)
...
test_that("create_analysis_meta defaults filters to empty list", {
	meta <- create_analysis_meta()

	expect_equal(meta@filters, list())
})

test_that("create_analysis_meta sets row_id correctly", {
	meta <- create_analysis_meta(row_id = "demographics_age_mean")

	expect_equal(meta@row_id, "demographics_age_mean")
})

test_that("create_analysis_meta defaults row_id to empty string", {
	meta <- create_analysis_meta()

	expect_equal(meta@row_id, "")
})

test_that("create_analysis_meta sets derivation correctly", {
	meta <- create_analysis_meta(derivation = "mean(AGE)")

	expect_equal(meta@derivation, "mean(AGE)")
})

test_that("create_analysis_meta defaults derivation to empty string", {
	meta <- create_analysis_meta()

	expect_equal(meta@derivation, "")
})

test_that("create_analysis_meta works with all parameters", {
	meta <- create_analysis_meta(
...
test_that("AnalysisMeta accepts NULL timestamp", {
	meta <- AnalysisMeta(
		source_vars = c("AGE"),
		timestamp = NULL
	)
...
test_that("AnalysisMeta accepts POSIXct timestamp", {
	ts <- Sys.time()
	meta <- AnalysisMeta(
		source_vars = c("AGE"),
		timestamp = ts
...
test_that("AnalysisMeta rejects non-POSIXct timestamp", {
	expect_error(
...
test_that("AnalysisMeta allows timestamp to be default (NULL)", {
	meta <- AnalysisMeta()
...
test_that("AnalysisMeta accepts character source_vars", {
	meta <- AnalysisMeta(source_vars = c("AGE", "SEX"))

	expect_equal(meta@source_vars, c("AGE", "SEX"))
})

test_that("AnalysisMeta defaults source_vars to empty character", {
	meta <- AnalysisMeta()

	expect_equal(meta@source_vars, character())
})

test_that("AnalysisMeta accepts list filters", {
...
test_that("AnalysisMeta defaults filters to empty list", {
	meta <- AnalysisMeta()

	expect_equal(meta@filters, list())
})

test_that("AnalysisMeta accepts character row_id", {
	meta <- AnalysisMeta(row_id = "test_row")

	expect_equal(meta@row_id, "test_row")
})

test_that("AnalysisMeta defaults row_id to empty string", {
	meta <- AnalysisMeta()

	expect_equal(meta@row_id, "")
})

test_that("AnalysisMeta accepts character derivation", {
	meta <- AnalysisMeta(derivation = "mean(x)")

	expect_equal(meta@derivation, "mean(x)")
})

test_that("AnalysisMeta defaults derivation to empty string", {
	meta <- AnalysisMeta()

	expect_equal(meta@derivation, "")
})

test_that("AnalysisMeta accepts empty package_version", {
	meta <- AnalysisMeta(package_version = "")

	expect_equal(meta@package_version, "")
})

test_that("AnalysisMeta accepts empty r_version", {
