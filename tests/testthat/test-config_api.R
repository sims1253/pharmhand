...
describe("config api", {
	it("parse_config_registry() creates valid registry from list", {
	...
	it("parse_config_registry() handles empty subgroups", {
		config_list <- list(
			subgroups = list(),
			populations = list(),
			soc_config = NULL,
	...
	it("parse_config_registry() handles empty populations", {
		config_list <- list(
			subgroups = list(),
			populations = list(),
			soc_config = NULL,
	...
	it("parse_config_registry() handles missing sections", {
		config_list <- list()
	...
	it("parse_config_registry() creates SOCConfig from YAML structure", {
		config_list <- list(
			soc_config = list(
				variable = "AEBODSYS",
				include_all = TRUE,
	...
	it("parse_config_registry() creates PTConfig from YAML structure", {
		config_list <- list(
			pt_config = list(
				variable = "AEDECOD",
				include_all = FALSE,
	...
	it("parse_config_registry() applies default values for missing optional
	fields", {
		config_list <- list(
			subgroups = list(
				minimal = list(
	...
	it("parse_config_registry() preserves custom performance settings", {
		config_list <- list(
			performance = list(
	...
	it("define_subgroup_config() adds new subgroup", {
	...
	it("define_subgroup_config() overrides existing subgroup", {
		registry <- ConfigurationRegistry()
		registry <- define_subgroup_config(
			registry,
			name = "age",
	...
	it("define_subgroup_config() respects custom priority", {
		registry <- ConfigurationRegistry()
		registry <- define_subgroup_config(
			registry,
			name = "test",
	...
	it("define_subgroup_config() handles empty labels", {
		registry <- ConfigurationRegistry()
		registry <- define_subgroup_config(
			registry,
			name = "no_labels",
	...
	it("define_subgroup_config() preserves order parameter", {
		registry <- ConfigurationRegistry()
		registry <- define_subgroup_config(
			registry,
			name = "ordered",
	...
	it("define_subgroup_config() preserves filter_values parameter", {
		registry <- ConfigurationRegistry()
		registry <- define_subgroup_config(
			registry,
			name = "filtered",
	...
	it("define_subgroup_config() throws error for invalid registry", {
		expect_error(
			define_subgroup_config(
				"not_a_registry",
				"name",
	...
	it("define_population_config() adds new population", {
		registry <- ConfigurationRegistry()
	...
	it("define_population_config() overrides existing population", {
		registry <- ConfigurationRegistry()
		registry <- define_population_config(
			registry,
			name = "SAF",
	...
	it("define_population_config() uses name as label if not provided", {
		registry <- ConfigurationRegistry()
		registry <- define_population_config(
			registry,
			name = "MY_POP",
	...
	it("define_population_config() respects custom flag_value", {
		registry <- ConfigurationRegistry()
		registry <- define_population_config(
			registry,
			name = "numeric_flag",
	...
	it("define_population_config() throws error for invalid registry", {
		expect_error(
			define_population_config(
				NULL,
				"name",
	...
	it("get_subgroup_config() returns existing subgroup", {
		config_list <- create_test_config()
		registry <- pharmhand:::parse_config_registry(config_list)
	...
	it("get_subgroup_config() returns NULL for non-existent subgroup", {
		registry <- ConfigurationRegistry()
	...
	it("get_subgroup_config() works after adding custom subgroup", {
		registry <- ConfigurationRegistry()
		registry <- define_subgroup_config(
			registry,
			name = "custom",
	...
	it("get_subgroup_config() throws error for invalid name type", {
		registry <- ConfigurationRegistry()
		expect_error(
	...
	it("get_population_config() returns existing population", {
		config_list <- create_test_config()
		registry <- pharmhand:::parse_config_registry(config_list)
	...
	it("get_population_config() returns NULL for non-existent population", {
		registry <- ConfigurationRegistry()
	...
	it("get_population_config() works after adding custom population", {
		registry <- ConfigurationRegistry()
		registry <- define_population_config(
			registry,
			name = "MY_POP",
	...
	it("get_population_config() throws error for invalid name type", {
		registry <- ConfigurationRegistry()
		expect_error(
	...
	it("list_subgroups() returns character vector", {
		config_list <- create_test_config()
		registry <- pharmhand:::parse_config_registry(config_list)
	...
	it("list_subgroups() returns expected subgroup names", {
		config_list <- list(
			subgroups = list(
	...
	it("list_subgroups() returns empty for registry with no subgroups", {
		registry <- ConfigurationRegistry()
		subgroups <- list_subgroups(registry)
	...
	it("list_subgroups() includes custom subgroups", {
		registry <- ConfigurationRegistry()
	...
	it("list_populations() returns character vector", {
		config_list <- create_test_config()
		registry <- pharmhand:::parse_config_registry(config_list)
	...
	it("list_populations() returns expected population names", {
		config_list <- list(
			populations = list(
	...
	it("list_populations() returns empty for registry with no populations", {
		registry <- ConfigurationRegistry()
		populations <- list_populations(registry)
	...
	it("list_populations() includes custom populations", {
		registry <- ConfigurationRegistry()
		registry <- define_population_config(
			registry,
			name = "pop1",
	...
	it("update_soc_config() updates existing SOC config", {
		config_list <- create_test_config()
		registry <- pharmhand:::parse_config_registry(config_list)
	...
	it("update_soc_config() creates new SOC config if none exists", {
		registry <- ConfigurationRegistry()
	...
	it("update_soc_config() can update multiple properties", {
		config_list <- create_test_config()
		registry <- pharmhand:::parse_config_registry(config_list)
	...
	it("update_soc_config() sets source to r_api", {
		config_list <- create_test_config()
		registry <- pharmhand:::parse_config_registry(config_list)
	...
	it("update_soc_config() sets priority to 100", {
		config_list <- create_test_config()
		registry <- pharmhand:::parse_config_registry(config_list)
	...
	it("update_soc_config() preserves existing values when updating some", {
		config_list <- create_test_config()
		registry <- pharmhand:::parse_config_registry(config_list)
	...
	it("update_soc_config() throws error for invalid registry", {
		expect_error(
	...
	it("update_pt_config() updates existing PT config", {
		config_list <- create_test_config()
		registry <- pharmhand:::parse_config_registry(config_list)
	...
	it("update_pt_config() creates new PT config if none exists", {
		registry <- ConfigurationRegistry()
	...
	it("update_pt_config() can update multiple properties", {
		config_list <- create_test_config()
		registry <- pharmhand:::parse_config_registry(config_list)
	...
	it("update_pt_config() sets source to r_api", {
		config_list <- create_test_config()
		registry <- pharmhand:::parse_config_registry(config_list)
	...
	it("update_pt_config() sets priority to 100", {
		config_list <- create_test_config()
		registry <- pharmhand:::parse_config_registry(config_list)
	...
	it("update_pt_config() preserves existing values when updating some", {
		config_list <- create_test_config()
		registry <- pharmhand:::parse_config_registry(config_list)
	...
	it("update_pt_config() throws error for invalid registry", {
		expect_error(
	...
	it("get_performance_setting() returns nested value with dot notation", {
		config_list <- create_test_config()
		registry <- pharmhand:::parse_config_registry(config_list)
	...
	it("get_performance_setting() returns default for missing setting", {
		config_list <- create_test_config()
		registry <- pharmhand:::parse_config_registry(config_list)
	...
	it("get_performance_setting() returns default for missing path", {
		config_list <- create_test_config()
		registry <- pharmhand:::parse_config_registry(config_list)
	...
	it("get_performance_setting() works with deeply nested settings", {
		config_list <- list(
			performance = list(
	...
	it("get_performance_setting() returns NULL if no default given", {
		config_list <- create_test_config()
		registry <- pharmhand:::parse_config_registry(config_list)
	...
	it("get_performance_setting() can access cache settings", {
		config_list <- list(
			performance = list(
	...
	it("get_performance_setting() can access parallel settings", {
		config_list <- list(
			performance = list(
	...
	it("get_performance_setting() handles single-level setting", {
		registry <- ConfigurationRegistry(
			performance = list(
	...
	it("get_performance_setting() throws error for invalid registry", {
		expect_error(
	...
	it("Runtime override has higher priority than YAML config", {
	...
	it("Multiple overrides maintain correct priority", {
		registry <- ConfigurationRegistry()
		registry <- define_subgroup_config(
			registry,
			name = "sg",
	...
	it("Configuration functions handle edge cases gracefully", {
	...
	it("Configuration preserves data integrity across operations", {
		config_list <- create_test_config()
		registry <- pharmhand:::parse_config_registry(config_list)
	...
	it("Custom settings are preserved through configuration operations", {
		registry <- ConfigurationRegistry(
			performance = list(
				custom = list(
					setting1 = "value1",
	...
	it("Full configuration round-trip works", {
		# Parse, modify, verify
		config_list <- create_test_config()
		registry <- pharmhand:::parse_config_registry(config_list)
	...
	it("load_config() can load from temporary YAML file", {
...
'
	...
	it("load_config() throws error for non-existent file", {
		expect_error(
	...
	it("load_config() returns registry with expected structure", {
