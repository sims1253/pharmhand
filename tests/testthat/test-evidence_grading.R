...
describe("evidence grading", {
	it("EvidenceGrade can be created with all required properties", {
		grade <- EvidenceGrade(
			grade = "proof",
			grade_de = "Beleg",
			direction = "benefit",
	...
	it("EvidenceGrade has correct default values", {
	...
	it("grade_evidence works with MetaResult (high quality - proof)", {
		meta_res <- MetaResult(
			estimate = 0.72,
			ci = c(0.62, 0.84),
			p_value = 0.0001,
	...
	it("grade_evidence works with MetaResult (moderate - indication)", {
		meta_res <- MetaResult(
			estimate = 0.78,
			ci = c(0.62, 0.98),
			p_value = 0.03,
	...
	it("grade_evidence works with MetaResult (low quality - hint)", {
		meta_res <- MetaResult(
			estimate = 0.82,
			ci = c(0.65, 1.04),
			p_value = 0.10,
	...
	it("grade_evidence works with MetaResult (no proof)", {
		meta_res <- MetaResult(
			estimate = 0.95,
			ci = c(0.75, 1.20),
			p_value = 0.67,
	...
	it("grade_evidence with MetaResult uses default RoB when NULL", {
		meta_res <- MetaResult(
			estimate = 0.70,
			ci = c(0.60, 0.82),
			p_value = 0.00001,
	...
	it("grade_evidence works with ComparisonResult (single study)", {
	...
	it("grade_evidence with ComparisonResult defaults n_studies to 1", {
		comp_res <- ComparisonResult(
			estimate = 0.80,
			ci = c(0.68, 0.94),
			p_value = 0.007,
	...
	it("grade_evidence with ComparisonResult uses n_studies_override", {
		comp_res <- ComparisonResult(
			estimate = 0.80,
			ci = c(0.68, 0.94),
			p_value = 0.007,
	...
	it("grade_evidence rejects non-MetaResult/ComparisonResult", {
		expect_error(
	...
	it("grade_evidence handles CI including null for proof downgrade", {
		meta_res <- MetaResult(
			estimate = 0.85,
			ci = c(0.70, 1.02),
			p_value = 0.08,
	...
	it("assess_evidence_domains returns all five domains", {
	...
	it("assess_evidence_domains assesses limitations with RoB results", {
		rob_results <- list(
			assess_rob2("Study1", "Low", "Low", "Low", "Low", "Low"),
			assess_rob2("Study2", "Low", "Low", "Low", "Low", "Low"),
			assess_rob2("Study3", "Low", "Low", "Low", "Low", "Low")
	...
	it("assess_evidence_domains handles single RoB2Result", {
		rob_result <- assess_rob2("Study1", "Low", "Low", "Low", "Low", "Low")
	...
	it("assess_evidence_domains handles mixed RoB judgments", {
		rob_results <- list(
			assess_rob2("Study1", "Low", "Low", "Low", "Low", "Low"),
	...
	it("assess_evidence_domains handles NULL rob_results", {
		domains <- assess_evidence_domains(
			estimate = 0.75,
			ci = c(0.60, 0.93),
			p_value = 0.008,
	...
	it("assess_evidence_domains handles inconsistency for single study", {
		domains <- assess_evidence_domains(
			estimate = 0.75,
			ci = c(0.60, 0.93),
			p_value = 0.008,
	...
	it("assess_evidence_domains assesses inconsistency by I2", {
	...
	it("assess_evidence_domains assesses imprecision by CI inclusion", {
	...
	it("assess_evidence_domains handles NA CI", {
		domains <- assess_evidence_domains(
			estimate = 0.75,
	...
	it("assess_evidence_domains assesses indirectness by score", {
		domains_low <- assess_evidence_domains(
			estimate = 0.75,
			ci = c(0.60, 0.93),
			p_value = 0.008,
	...
	it("assess_evidence_domains defaults indirectness to low when NULL", {
		domains <- assess_evidence_domains(
			estimate = 0.75,
			ci = c(0.60, 0.93),
			p_value = 0.008,
	...
	it("assess_evidence_domains assesses publication bias", {
	...
	it("assess_evidence_domains handles <5 studies for publication bias", {
		domains <- assess_evidence_domains(
			estimate = 0.75,
			ci = c(0.60, 0.93),
			p_value = 0.008,
	...
	it("format_evidence_grade works for all four grades in English", {
	...
	it("format_evidence_grade works for all four grades in German", {
		grades <- c("proof", "indication", "hint", "none")
	...
	it("format_evidence_grade text format with details", {
		grade <- EvidenceGrade(
			grade = "indication",
			grade_de = "Hinweis",
			direction = "benefit",
	...
	it("format_evidence_grade HTML format", {
		grade <- EvidenceGrade(
			grade = "proof",
			grade_de = "Beleg",
			direction = "benefit",
	...
	it("format_evidence_grade LaTeX format", {
		grade <- EvidenceGrade(
			grade = "hint",
	...
	it("format_evidence_grade without details returns just grade name", {
		grade <- EvidenceGrade(
			grade = "indication",
			grade_de = "Hinweis",
			direction = "benefit",
	...
	it("format_evidence_grade rejects non-EvidenceGrade object", {
		expect_error(
	...
	it("format_evidence_grade rejects invalid language", {
	...
	it("format_evidence_grade rejects invalid format", {
		grade <- EvidenceGrade(grade = "proof", grade_de = "Beleg")
	...
	it("print.EvidenceGrade displays grade information", {
		grade <- EvidenceGrade(
			grade = "indication",
			grade_de = "Hinweis",
			direction = "benefit",
	...
	it("print.EvidenceGrade returns object invisibly", {
		grade <- EvidenceGrade(grade = "proof", grade_de = "Beleg")
	...
	it("evidence_summary_table works with EvidenceGrade objects", {
		grades <- list(
			OS = EvidenceGrade(
				grade = "proof",
				grade_de = "Beleg",
	...
	it("evidence_summary_table works with German output", {
		grades <- list(
			OS = EvidenceGrade(
				grade = "proof",
				grade_de = "Beleg",
	...
	it("evidence_summary_table works with data frame input", {
		df <- data.frame(
			outcome = c("OS", "PFS"),
	...
	it("evidence_summary_table handles mixed list/data frame inputs", {
		grades <- list(
			list(
				outcome = "OS",
				grade_en = "proof",
	...
	it("evidence_summary_table requires names or outcomes", {
		grades <- list(
	...
	it("evidence_summary_table rejects non-list/non-data.frame input", {
		expect_error(
	...
	it("All four evidence grades can be produced by grade_evidence", {
	...
	it("handles all-NA ratings without paradoxical certainty", {
