...
test_that("CountLayer creates valid object", {
  layer <- CountLayer(
    target_var = "SEX",
    by_vars = character(),
    distinct_by = "USUBJID",
...
test_that("DescriptiveLayer creates valid object", {
...
test_that("add_layer works correctly", {
  tbl <- LayeredTable(
    data = data.frame(x = 1:10, trt = rep(c("A", "B"), 5)),
    trt_var = "trt"
  )
...
test_that("add_layer rejects invalid input", {
  tbl <- LayeredTable(
...
test_that("build_layer computes correct counts", {
  data <- data.frame(
...
test_that("build_layer computes correct descriptive stats", {
  data <- data.frame(
...
test_that("build_table combines layers correctly", {
  data <- data.frame(
    USUBJID = paste0("0", 1:6),
    TRT = c("A", "A", "A", "B", "B", "B"),
    SEX = c("M", "M", "F", "M", "F", "F"),
...
test_that("LayeredTable validates layers property", {
  expect_error(
    LayeredTable(
      data = data.frame(x = 1),
      trt_var = "x",
