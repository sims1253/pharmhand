...
describe("bias rob2", {
	it("RoB2Result can be created with all required properties", {
		result <- RoB2Result(
			study_id = "STUDY001",
			domains = list(
				...
				D5_selection = list(
					judgment = "Low",
					support = "Pre-specified analysis"
				)
			),
			overall = "Low",
	...
	it("RoB2Result rejects missing domain", {
		expect_error(
			RoB2Result(
				study_id = "STUDY001",
				domains = list(
	...
	it("RoB2Result rejects domain without judgment", {
		expect_error(
			RoB2Result(
				study_id = "STUDY001",
				domains = list(
	...
	it("RoB2Result rejects invalid domain judgment", {
		expect_error(
			RoB2Result(
				study_id = "STUDY001",
				domains = list(
	...
	it("RoB2Result rejects invalid overall judgment", {
		expect_error(
			RoB2Result(
				study_id = "STUDY001",
				domains = list(
	...
	it("RoB2Result has correct default values", {
		result <- RoB2Result(
			study_id = "STUDY001",
			domains = list(
	...
	it("RoB2Result summary_df computed property works", {
		result <- RoB2Result(
			study_id = "STUDY001",
			domains = list(
	...
	it("RoB2Result judgment_counts computed property works", {
		result <- RoB2Result(
			study_id = "STUDY001",
			domains = list(
				D1_randomization = list(judgment = "Low"),
	...
	it("assess_rob2 creates valid result with all Low judgments", {
		result <- assess_rob2(
			study_id = "STUDY001",
	...
	it("assess_rob2 auto-calculates overall as High with any High domain", {
		result <- assess_rob2(
			study_id = "STUDY001",
	...
	it("assess_rob2 auto-calculates overall as High with 2+ Some concerns", {
	...
	it("assess_rob2 auto-calculates overall as Some concerns with
           1 Some concerns", {
	...
	it("assess_rob2 auto-calculates overall correctly for mixed scenarios", {
	...
	it("assess_rob2 accepts custom overall judgment", {
		result <- assess_rob2(
			study_id = "STUDY001",
			d1_randomization = "Low",
			d2_deviations = "Low",
	...
	it("assess_rob2 accepts optional outcome parameter", {
		result <- assess_rob2(
			study_id = "STUDY001",
			d1_randomization = "Low",
			d2_deviations = "Low",
	...
	it("assess_rob2 accepts optional support text", {
		result <- assess_rob2(
			study_id = "STUDY001",
			d1_randomization = "Low",
			d2_deviations = "Low",
	...
	it("assess_rob2 accepts optional metadata", {
		result <- assess_rob2(
			study_id = "STUDY001",
			d1_randomization = "Low",
			d2_deviations = "Low",
	...
	it("assess_rob2 rejects missing study_id", {
		expect_error(
			assess_rob2(
				study_id = NULL,
				d1_randomization = "Low",
	...
	it("assess_rob2 rejects invalid domain judgment", {
		expect_error(
			assess_rob2(
				study_id = "STUDY001",
	...
	it("assess_rob2 rejects NULL domain judgment", {
	...
	it("assess_rob2 rejects empty string domain judgment", {
		expect_error(
			assess_rob2(
				study_id = "STUDY001",
				d1_randomization = "",
	...
	it(".calculate_rob2_overall returns Low for all Low judgments", {
		judgments <- c("Low", "Low", "Low", "Low", "Low")
		expect_equal(.calculate_rob2_overall(judgments), "Low")
	})

	it(".calculate_rob2_overall returns High for any High judgment", {
		# One High
	...
	it(".calculate_rob2_overall returns High for 2+ Some concerns", {
		# Exactly 2
	...
	it(".calculate_rob2_overall returns Some concerns for 1 Some concerns", {
		judgments <- c("Some concerns", "Low", "Low", "Low", "Low")
		expect_equal(.calculate_rob2_overall(judgments), "Some concerns")
	})

	it(".calculate_rob2_overall rejects wrong length input", {
		expect_error(
	...
	it(".generate_rob2_justification returns correct text for Low", {
		judgments <- c("Low", "Low", "Low", "Low", "Low")
		just <- .generate_rob2_justification(judgments, "Low")
		expect_equal(just, "All domains rated Low risk of bias")
	})

	it(".generate_rob2_justification returns correct text for High
  with High domain", {
  ...
  with Some concerns", {
	...
	it(".generate_rob2_justification returns correct text for High with
           both", {
	...
	it(".generate_rob2_justification returns correct text for Some
           concerns", {
	...
	it("assess_rob2_batch processes single study data frame", {
		data <- data.frame(
			study_id = "STUDY001",
			d1_randomization = "Low",
			d2_deviations = "Low",
	...
	it("assess_rob2_batch processes multiple studies", {
		data <- data.frame(
	...
	it("assess_rob2_batch includes optional columns", {
		data <- data.frame(
			study_id = "STUDY001",
			d1_randomization = "Low",
			d2_deviations = "Low",
	...
	it("assess_rob2_batch accepts custom overall judgments", {
		data <- data.frame(
			study_id = "STUDY001",
			d1_randomization = "Low",
			d2_deviations = "Low",
	...
	it("assess_rob2_batch handles missing optional columns gracefully", {
		data <- data.frame(
			study_id = "STUDY001",
			d1_randomization = "Low",
			d2_deviations = "Low",
	...
	it("assess_rob2_batch rejects data frame without study_id column", {
	...
	it("assess_rob2_batch rejects data frame without required domain
           columns", {
	...
	it("assess_rob2_batch rejects non-data-frame input", {
		expect_error(
	...
	it("rob2_summary creates data frame from single result", {
		results <- list(
			assess_rob2(
				study_id = "STUDY001",
				d1_randomization = "Low",
	...
	it("rob2_summary creates data frame from multiple results", {
		results <- list(
			assess_rob2(
				study_id = "STUDY001",
				d1_randomization = "Low",
	...
	it("rob2_summary includes justification when requested", {
		results <- list(
			assess_rob2(
				study_id = "STUDY001",
				d1_randomization = "High",
	...
	it("rob2_summary returns empty data frame for empty list", {
	...
	it("rob2_summary rejects non-list input", {
		expect_error(
	...
	it("rob2_summary rejects list with non-RoB2Result elements", {
		results <- list(
			assess_rob2(
				study_id = "STUDY001",
				d1_randomization = "Low",
	...
	it("RoB2Result print method works", {
		result <- assess_rob2(
			study_id = "STUDY001",
			d1_randomization = "Low",
			d2_deviations = "Low",
	...
	it("All three judgments are valid for all domains", {
		skip_on_cran()
	...
	it("assess_rob2 handles NA in optional parameters", {
	...
	it("assess_rob2 handles character study_id correctly", {
		result <- assess_rob2(
