...
describe("efficacy subgroup", {
	it("create_subgroup_analysis_table works", {
	...
	it("create_subgroup_analysis_table warns for small subgroups", {
	...
	it("create_subgroup_analysis_table pulls subgroup vars from adsl", {
		adsl <- data.frame(
			USUBJID = c("01", "02"),
			SEX = c("M", "F")
		)
	...
	it("create_subgroup_analysis_table errors on duplicate adsl USUBJID", {
		adsl <- data.frame(
			USUBJID = c("01", "01"),
			SEX = c("M", "F")
		)
	...
	it("create_subgroup_analysis_table errors on missing subgroup vars", {
		adsl <- data.frame(USUBJID = c("01", "02"))
		advs <- data.frame(
			USUBJID = c("01", "02"),
			TRT01P = c("A", "B"),
	...
	it("create_subgroup_analysis_table validates inputs", {
		adsl <- data.frame(USUBJID = c("01", "02"))
	...
	it("create_subgroup_table works with TTE endpoint", {
	...
	it("create_subgroup_table works with binary endpoint", {
		set.seed(42)
		adrs <- data.frame(
	...
	it("create_subgroup_table includes interaction p-values", {
		skip_if_not_installed("survival")
	...
	it("create_subgroup_table can disable interaction p-values", {
		skip_if_not_installed("survival")
	...
	it("create_subgroup_table warns for small subgroups", {
		adrs <- data.frame(
	...
	it("create_subgroup_table warns for missing subgroup variables", {
		skip_if_not_installed("survival")
	...
	it("create_subgroup_table with empty subgroup levels", {
	...
	it("create_subgroup_table with multiplicity adjustment", {
		adtte <- data.frame(
	...
	it("create_subgroup_table disables multiplicity adjustment", {
		adtte <- data.frame(
			USUBJID = sprintf("SUBJ%02d", 1:60),
			TRT01P = rep(c("Placebo", "Active"), each = 30),
			AVAL = c(rexp(30, 0.05), rexp(30, 0.03)),
	...
	it("create_subgroup_table handles small subgroup with binary endpoint", {
		adrs <- data.frame(
			USUBJID = sprintf("SUBJ%02d", 1:30),
			TRT01P = rep(c("Placebo", "Active"), each = 15),
	...
	it("create_subgroup_table with binary endpoint handles no events", {
